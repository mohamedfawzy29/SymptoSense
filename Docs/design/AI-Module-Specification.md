...
Module: AI Understanding & Communication Version: 2 Status: Draft — Aligned with Approved AI Requirements, Business Rules, and Error & Edge Cases documents
...
Unlike the 3D Module's one-time input step, the AI Module is invoked on every conversational turn for the remainder of the assessment. It has no ability to decide when the conversation ends — that determination belongs entirely to the Assessment Module (BR-013, BR-018).
...
The calling layer (Conversation Orchestrator — ownership still open, see Open Item 1) is responsible for maintaining the ongoing conversation state and for resolving assessmentId to a user account when persistence is required. The AI Module never performs that resolution itself.
...
Whether this configuration lives in application config, a shared config store, or a small lookup service is an implementation decision — see Open Item 3.
...
Application
 |
 v
IAiProviderAdapter (Section 18)
 |
 +---- QwenAdapter (AWS Bedrock)
 +---- LlamaAdapter (AWS Bedrock)
 +---- MedGemmaAdapter (AWS SageMaker, pending confirmation — see Open Item 4)
...
Response: the Structured Output JSON shown in Section 8.
...
Final route naming and payload shape remain an implementation detail, as with the 3D Module's API.
...
No dependency on ASP.NET, Entity Framework, HTTP, controllers, databases, or any specific AI provider SDK.
...
IStructuredOutputValidator.Validate is what rejects out-of-range values (e.g., Severity = 15) before UnderstandInputResponseDto is ever returned to the caller (ERR-002).
...
QwenBedrockAdapter, LlamaBedrockAdapter, and MedGemmaAdapter each implement IAiProviderAdapter against their respective AWS endpoint (Bedrock Runtime or SageMaker Runtime). Swapping the active provider during the model evaluation phase (AI-017, AI-018) requires only a configuration change, not a change to the Application or Domain layers.
...
The controller delegates directly to the Application layer; no validation or business logic lives in the controller itself.

20. Open Items Requiring Team Confirmation
Conversation Orchestrator ownership. Something must own the multi-turn loop shown in Section 6 (call AI, call Assessment, repeat) and the short-lived conversation state behind it. This document assumes that component sits outside the AI Module — confirm who owns it and where its state lives (likely Redis, per the earlier architecture discussion).
Full extraction schema. RESOLVED — the team has confirmed the six MVP symptom cases (Headache, Fever, Cough, Abdominal Pain, Chest Pain, Back Pain); see Section 3.2. Extending beyond these six later is a content/config task, not a schema change.
AI provider configuration store. Whether prompt templates and provider endpoint configuration live in application config, a shared config store, or a small lookup service is still open.
MedGemma hosting path. Confirm whether MedGemma will be reachable via a managed AWS offering or requires self-hosted GPU infrastructure once the model evaluation concludes.
Output Validator boundary. Confirm whether IStructuredOutputValidator lives only inside the AI Module (current assumption) or is shared with the Assessment Module as a second, independent check — defense in depth may be worth the duplication given the safety stakes.
21. Summary
...
The AI Module runs on every conversational turn until the Assessment Module signals completion. It never determines that completion itself, never evaluates safety, and never modifies a result it did not produce.